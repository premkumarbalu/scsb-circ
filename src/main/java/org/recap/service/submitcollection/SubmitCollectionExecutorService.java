...
/*        for (BibRecord bibRecord : bibRecords.getBibRecords()) {
    ...
    public String loadRecord(Set<Integer> processedBibIds, Map<String, List<SubmitCollectionReportInfo>> submitCollectionReportInfoMap, List<Map<String, String>> idMapToRemoveIndexList, boolean isCGDProtected, InstitutionEntity institutionEntity, String format, List<Record> recordList, int count, Set<String> processedBarcodeSetForDummyRecords) throws Exception{
        StopWatch stopWatch = new StopWatch();
        stopWatch.start();
        //ExecutorService executorService = Executors.newFixedThreadPool(submitCollectionThreadSize);
        logger.info("inside SubmitCollectionExecutorService loadRecord");
        List<Callable<BibliographicEntity>> callables = new ArrayList<>();
        for(Record record : recordList){
            SubmitCollectionCallable submitCollectionCallable = applicationContext.getBean(SubmitCollectionCallable.class,record,format, submitCollectionReportInfoMap, idMapToRemoveIndexList,isCGDProtected,institutionEntity,processedBarcodeSetForDummyRecords);
            callables.add(submitCollectionCallable);
        }
        try {
            List<Future<BibliographicEntity>> futureList = getExecutorService().invokeAll(callables);
